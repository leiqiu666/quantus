...
根据 SDD spec 中的参数（接口名、domain、表名、拉取模式、冲突键、字段列表等），
自动生成 8 层代码骨架文件（含 TODO 标记），并输出需要手动添加到现有文件的代码片段。

用法：
    uv run python .claude/skills/vibe_tushare_etl/generate_skeleton.py \\
      --api-name suspend_d \\
      --domain stock_suspend \\
      --domain-dir stock \\
      --table-name stock_suspend \\
      --pull-mode by-date \\
      --conflict-keys "ts_code,trade_date,suspend_type,suspend_timing" \\
      --output-fields "ts_code:str,trade_date:str,suspend_type:str,suspend_timing:str" \\
      --rate-limit 200 \\
      --has-transform false \\
      --has-completeness false \\
      --cli-group suspend \\
      --cli-command pull-by-date \\
      --spec-path spec/etl/基础-A股停复牌数据入库.sdd.md
"""
...
class {self.domain_cls}Strategy:
    def __init__(self) -> None:
        ...
        self.{self.domain}_start_date = settings.{self.domain}_start_date
