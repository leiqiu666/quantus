...
> **CLI 命令：** `{{cli_group}} {{cli_command}}`；`{{cli_group}} check-complete`（若启用完整性）
> **Admin 大类：** {{dashboard_group_title}}（`group_id={{dashboard_group_id}}`）
> **SSE task_key：** `{{sse_task_key}}`
> **源码入口：** `src/etl/cli.py`
...
| 项 | 值 |
|----|-----|
| 大类 / 分组 | `{{dashboard_group_id}}` — {{dashboard_group_title}} |
| 明细页 | `{{dashboard_detail_path}}` |
| 日期维度 | `{{date_key_type}}`（{{date_label}}） |
| 看板列 key | `{{column_key}}` |
| 列头 label | `{{column_label}}` |
| 完整率阈值 | {{threshold}} |
| SSE task_key | `{{sse_task_key}}` |
...
**不新建**独立 ProTable 页；列追加到已有宽表 [`DataSourceDashboardTable`](../../src/web/admin/src/components/DataSourceDashboard/DataSourceDashboardTable.tsx)。
...
| Strategy 方法 | `check_complete(start_date, end_date)` |
| 宏观快照 | `CompletenessEngine.refresh_snapshot` |
| 微观补拉 | `CompletenessEngine.check_complete` → `pull_by_date` |
...
| 项 | 值 |
|----|-----|
| 通用补位 | `POST /api/admin/etl/sse/run` |
| task_key | `{{sse_task_key}}` |
| 注册表 | [`etl_sse_registry.py`](../../src/service/etl/etl_sse_registry.py) |
| Strategy 复用 | 与 CLI `check-complete` 同一方法 |

Spec：[`spec/api/admin/ETL-补位-SSE.sdd.md`](../../spec/api/admin/ETL-补位-SSE.sdd.md)。
