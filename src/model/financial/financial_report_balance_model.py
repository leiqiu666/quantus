...
class ReportBalanceModel:
    ...
    def get_period_list(
        ...
        *,
        ...
        逻辑:
            1. report_balance 表中 end_date 为报告期
            2. 按 end_date 分组统计条数，可按起止报告期筛选
        Returns:
            报告期列表（倒序），元素含 report_period、report_balance_count
        """
        clauses = [
            ReportBalanceEntities.end_date.isnot(None),
            ReportBalanceEntities.end_date != "",
        ]
        if start_period_date is not None:
            clauses.append(ReportBalanceEntities.end_date >= start_period_date)
        if end_period_date is not None:
            clauses.append(ReportBalanceEntities.end_date <= end_period_date)
        rows = self.db.select_grouped(
            ReportBalanceEntities,
            ReportBalanceEntities.end_date,
            func.count(ReportBalanceEntities.id),
            group_by=(ReportBalanceEntities.end_date,),
            order_by=(ReportBalanceEntities.end_date.desc(),),
            where_clauses=tuple(clauses),
        )
        return [
            {"report_period": end_date, "report_balance_count": int(cnt)}
    ...
    def get_report_balance_all(self, *, return_fields: tuple[str, ...] | None = None, **kwargs):
        """
        获取资产负债表全量数据（无筛选条件时即整表；数据量可能很大，请谨慎使用）。
        Args:
        ...
        if return_fields is None:
            return self.db.get_all(ReportBalanceEntities, **kwargs)
        if not return_fields:
            raise ValueError("return_fields 不能为空元组")
        return self.db.fetch_model_columns(ReportBalanceEntities, return_fields, **kwargs)

    def list_by_end_date(self, end_date: str) -> List[Dict[str, Any]]:
        """返回指定报告期全行（dict 列表，供 Load filter 比对）。"""
        rows = self.db.get_all(ReportBalanceEntities, end_date=str(end_date).strip())
        columns = ReportBalanceEntities.__table__.columns
        return [
...
if __name__ == "__main__":
    report_balance_model = ReportBalanceModel()
    print(report_balance_model.get_period_list())
