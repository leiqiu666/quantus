...
class FinaMainbzLocalExtract:
    ...
    def get_max_end_date(self) -> str | None:
        ...
        try:
            row = session.query(func.max(FinaMainbzEntities.end_date)).scalar()
            if row is None:
                return None
            s = str(row).strip()
    ...
    def load_ts_codes_by_periods(self, periods: list[str]) -> dict[str, set[str]]:
        ...
        try:
            rows = (
                session.query(FinaMainbzEntities.end_date, FinaMainbzEntities.ts_code)
                .filter(FinaMainbzEntities.end_date.in_(periods))
                .all()
            )
