...
TOP10_FLOATHOLDERS_COLUMNS: tuple[str, ...] = (
    "ts_code",
    "ann_date",
    "end_date",
    "holder_name",
...
def finalize_top10_floatholders(df: pd.DataFrame | None) -> pd.DataFrame:
    ...
    for col in TOP10_FLOATHOLDERS_COLUMNS:
        if col not in out.columns:
            out[col] = pd.NA if col not in _DATE_COLS else ""

    return out[list(TOP10_FLOATHOLDERS_COLUMNS)].reset_index(drop=True)
