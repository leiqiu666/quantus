"""前十大股东 Strategy：按公告日编排、95% 完整性守门（event_driven）。"""
...
from src.etl.workflow.financial.financial_shareholder_workflow import ShareholderWorkflow
...
class ShareholderStrategy:
    def __init__(self) -> None:
        self.shareholder_workflow = ShareholderWorkflow()
        self.trade_cal_strategy = TradeCalStrategy()
        self.shareholder_start_date = settings.etl_start_date("financial_shareholder_top10")

    def pull_top10_holders_by_date(
        self,
        start_date: str | None = None,
        end_date: str | None = None,
    ) -> int:
        if start_date is None:
            start_date = self.shareholder_start_date
        if end_date is None:
        ...
        if not pending:
            print(f"[信息] {floor}~{end} 前十大股东已完整，跳过")
            return 0
        ...
        total_saved = 0
        for ann_date in tqdm_iter(pending, desc="前十大股东入库", unit="日"):
            n = self.shareholder_workflow.pull_top10_by_ann_date(ann_date=ann_date)
            total_saved += n
            if n == 0:
    ...
    def _completeness(self) -> CompletenessEngine:
        return CompletenessEngine(CompletenessConfig(
            source_name="financial_shareholder_top10",
            entity_class=Top10HoldersEntities,
            date_column="ann_date",
            start_date=self.shareholder_start_date,
            event_driven=True,
            pull_by_date=lambda td: self.shareholder_workflow.pull_top10_by_ann_date(
                ann_date=td
            ),
        ))
