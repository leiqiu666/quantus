"""财报披露计划 Strategy：按报告期全市场拉取、95% 完整性守门。"""
...
from src.etl.workflow.financial.financial_disclosure_date_workflow import DisclosureDateWorkflow
...
class DisclosureDateStrategy:
    def __init__(self) -> None:
        self.workflow = DisclosureDateWorkflow()
        self.start_date = settings.etl_start_date("financial_disclosure_date")

    def pull_disclosure_date_by_period(
        self,
        ...
        if not pending:
            print(f"[信息] {floor}~{end} 财报披露计划已完整（≥95%），跳过")
            return 0
        ...
        total_saved = 0
        for period in tqdm_iter(pending, desc="财报披露计划入库", unit="期"):
            n = self.workflow.pull_disclosure_date_by_period(period)
            total_saved += n
    ...
    def _completeness(self) -> CompletenessEngine:
        return CompletenessEngine(CompletenessConfig(
            ...
            entity_class=DisclosureDateEntities,
            date_column="end_date",
            start_date=self.start_date,
            is_period=True,
            pull_by_date=lambda p: self.workflow.pull_disclosure_date_by_period(p),
        ))
