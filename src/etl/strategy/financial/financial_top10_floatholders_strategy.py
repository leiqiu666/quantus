"""前十大流通股东 Strategy：按公告日编排、95% 完整性守门（event_driven）。"""
...
from src.etl.workflow.financial.financial_top10_floatholders_workflow import (
    Top10FloatholdersWorkflow,
)


class Top10FloatholdersStrategy:
    def __init__(self) -> None:
        self.workflow = Top10FloatholdersWorkflow()
        self.trade_cal_strategy = TradeCalStrategy()
        self.start_date = settings.etl_start_date("financial_top10_floatholders")

    def pull_top10_floatholders_by_date(
        self,
        ...
        if not pending:
            print(f"[信息] {floor}~{end} 前十大流通股东已完整，跳过")
            return 0
        ...
        total_saved = 0
        for ann_date in tqdm_iter(pending, desc="前十大流通股东入库", unit="日"):
            n = self.workflow.pull_top10_floatholders_by_ann_date(ann_date=ann_date)
            total_saved += n
            if n == 0:
    ...
    def _completeness(self) -> CompletenessEngine:
        return CompletenessEngine(CompletenessConfig(
            ...
            entity_class=Top10FloatholdersEntities,
            date_column="ann_date",
            start_date=self.start_date,
            event_driven=True,
            pull_by_date=lambda td: self.workflow.pull_top10_floatholders_by_ann_date(
                ann_date=td
            ),
        ))
