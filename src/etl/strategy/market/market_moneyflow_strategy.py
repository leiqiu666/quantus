"""个股资金流向 Strategy：按 SSE 开市日编排、95% 完整性守门、tqdm 循环。"""
...
from src.etl.workflow.market.market_moneyflow_workflow import MoneyflowWorkflow
...
class MoneyflowStrategy:
    def __init__(self) -> None:
        self.moneyflow_workflow = MoneyflowWorkflow()
        self.trade_cal_strategy = TradeCalStrategy()
        self.moneyflow_start_date = settings.etl_start_date("market_moneyflow")

    def pull_moneyflow_by_date_range(
        self,
        start_date: str | None = None,
        end_date: str | None = None,
    ) -> int:
        if start_date is None:
            start_date = self.moneyflow_start_date
        if end_date is None:
        ...
        if not pending:
            print(f"[信息] {floor}~{end} moneyflow 已完整（≥95%），跳过")
            return 0
        ...
        total_saved = 0
        for td in tqdm_iter(pending, desc="资金流向入库", unit="日"):
            n = self.moneyflow_workflow.pull_moneyflow_by_date(trade_date=td)
            total_saved += n
    ...
    def _completeness(self) -> CompletenessEngine:
        return CompletenessEngine(CompletenessConfig(
            source_name="market_moneyflow",
            entity_class=MoneyflowEntities,
            date_column="trade_date",
            start_date=self.moneyflow_start_date,
            threshold=0.92,
            pull_by_date=lambda td: self.moneyflow_workflow.pull_moneyflow_by_date(trade_date=td),
        ))
