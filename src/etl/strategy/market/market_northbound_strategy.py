...
from src.etl.workflow.market.market_northbound_workflow import HsgtWorkflow
...
class HsgtStrategy:
    def __init__(self) -> None:
        self.hsgt_workflow = HsgtWorkflow()
        self.trade_cal_strategy = TradeCalStrategy()
        self.hsgt_start_date = settings.etl_start_date("market_northbound_top10")

    def pull_hsgt_top10_by_date_range(
        self,
        start_date: str | None = None,
        end_date: str | None = None,
    ) -> int:
        if start_date is None:
            start_date = self.hsgt_start_date
        if end_date is None:
        ...
        if not pending:
            print(f"[信息] {floor}~{end} hsgt_top10 已完整（≥95%），跳过")
            return 0
        ...
        total_saved = 0
        for td in tqdm_iter(pending, desc="沪深股通十大成交入库", unit="日"):
            n = self.hsgt_workflow.pull_hsgt_top10_by_date(trade_date=td)
            total_saved += n
    ...
    def _completeness(self) -> CompletenessEngine:
        return CompletenessEngine(CompletenessConfig(
            source_name="market_northbound_top10",
            entity_class=HsgtTop10Entities,
            date_column="trade_date",
            start_date=self.hsgt_start_date,
            pull_by_date=lambda td: self.hsgt_workflow.pull_hsgt_top10_by_date(trade_date=td),
        ))
